...
public struct DocumentAsnEditingView<DocumentType>: View
where DocumentType: DocumentProtocol & Equatable {
  @Binding public var document: DocumentType
  ...
  public init(document: Binding<DocumentType>, isValid: Binding<Bool>) {
    _document = document
    _isValid = isValid
    let asn =
      if let asn = self.document.asn {
        String(asn)
      } else {
        ""
      }
