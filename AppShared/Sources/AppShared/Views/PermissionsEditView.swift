...
where T: Identifiable & LocalizedResource, T.ID == UInt, E: PermissionsModel {
  ...
  @Binding public var object: E

  // kind: read or write
  public let kind: WritableKeyPath<Permissions, Permissions.Set>
  // type: user os group
  public let type: WritableKeyPath<Permissions.Set, [UInt]>
  ...
  public var body: some View {
    Form {
      ...
      if !store.permissions.test(.view, for: .user) {
        ...
        if case .user(let id) = object.owner, id != store.currentUser?.id {
          Row(isActive: true) {
...
@MainActor
public struct PermissionsEditView<Object>: View where Object: PermissionsModel {
  @Binding public var object: Object
  @State public var original: Object

  public init(object: Binding<Object>) {
    _object = object
  ...
  public var body: some View {
    ...
    .task {
      do {
        ...
        document = try await store.repository.documents(filter: .default).fetch(limit: 100_000)
          .first { $0.title == "blubb" }
