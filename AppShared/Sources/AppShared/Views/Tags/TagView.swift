...
public func blend(_ a: Color, _ b: Color, by t: Double) -> Color {
  var ar: CGFloat = 0
  var ag: CGFloat = 0
  var ab: CGFloat = 0
  var aa: CGFloat = 0
...
public struct TagView<Trailing: View>: View {
  ...
  public init(tag: Tag? = nil) where Trailing == EmptyView {
    inputTag = tag
    trailing = EmptyView()
  }

  public init(tag: Tag? = nil, @ViewBuilder trailing: () -> Trailing) {
    inputTag = tag
...
public struct TagsView<Content: View>: View {
  ...
  public init(tags: [Tag?], action: ((Tag) -> Void)?, @ViewBuilder content: @escaping () -> Content)
  {
    self.tags = tags
    self.action = action
...
extension TagsView where Content == EmptyView {
  public init(tags: [Tag?], action: ((Tag) -> Void)? = nil) {
    self.tags = tags
    self.action = action
