//
//  Binding+CasePath.swift
//  swift-paperless
//
//  Created by Paul Gessinger on 06.07.25.
//

// Modified from https://raw.githubusercontent.com/pointfreeco/swiftui-navigation/refs/heads/main/Sources/SwiftUINavigation/Binding.swift
    ...
    public extension Binding {
            ...
            /// - Parameter keyPath: A case key path to a specific associated value.
            /// - Returns: A new binding.
            subscript<Enum: CasePathable, Member>(
        ...
        init(unwrapping base: Binding<Value?>, default value: Value) {
...
//    }
    ...
    fileprivate extension CasePathable {
        subscript<Member>(
    ...
    fileprivate extension Optional where Wrapped: CasePathable {
        subscript<Member>(
            keyPath: KeyPath<Wrapped.AllCasePaths, AnyCasePath<Wrapped, Member>>
        ) -> Member? {
            get {
                flatMap(Wrapped.allCasePaths[keyPath: keyPath].extract(from:))
            }
            set {
                let casePath = Wrapped.allCasePaths[keyPath: keyPath]
                guard flatMap(casePath.extract(from:)) != nil
                else { return }
