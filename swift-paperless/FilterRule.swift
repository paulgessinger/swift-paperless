...
extension KeyedDecodingContainerProtocol {
  ...
  where T: Decodable, T: LosslessStringConvertible {
    ...
    guard let value = T(s) else {
      throw DecodingError.typeMismatch(
        type, .init(codingPath: [key], debugDescription: "Could not be converted from string"))
    }
    return value
  }
  ...
  where T: Decodable, T: LosslessStringConvertible {
    guard let value = try decodeOrConvertOptional(type, forKey: key) else {
      throw DecodingError.typeMismatch(
        type, .init(codingPath: [key], debugDescription: "Nil value but no nullable value expected")
      )
    }
    return value
  }
  ...
  static func queryItems(for rules: [FilterRule]) -> [URLQueryItem] {
    ...
    for (type, group) in groups {
      ...
      if let filterVar = type.filterVar() {
        ...
        Logger.shared.warning(
          "Unable to add query item for \(String(reflecting: type), privacy: .public)")
      }
    }
    ...
    for rule in rules.filter({ !$0.ruleType.multiple() }) {
      guard let filterVar = rule.ruleType.filterVar() else {
        Logger.shared.warning(
          "Unable to add query item for \(String(reflecting: rule.ruleType), privacy: .public)")
        continue
      }
  ...
  init(from decoder: any Decoder) throws {
    ...
    case .date:
      ...
      guard let date = dateFormatter.date(from: dateStr) else {
        Logger.shared.error(
          "Unable to decode filter rule date string: \(dateStr, privacy: .public)")
...
struct FilterState: Equatable, Codable, Sendable {
  ...
  var defaultSorting: Bool {
    sortField == AppSettings.value(for: .defaultSortField, or: .added)
      && sortOrder == AppSettings.value(for: .defaultSortOrder, or: .descending)
  }
    ...
    for rule in rules {
      ...
      case .ownerAny:
        ...
        case .noneOf, .notAssigned:
          let ownerCopy = owner
          Logger.shared.error(
            "Owner is already set to \(String(describing: ownerCopy)), but got rule ownerAny=\(id)")
          fallthrough  // reset anyway
        case .any:
          owner = .anyOf(ids: [id])
        }
      ...
      case .ownerDoesNotInclude:
        ...
        case .anyOf, .notAssigned:
          let ownerCopy = owner
          Logger.shared.error(
            "Owner is already set to \(String(describing: ownerCopy)), but got rule ownerDoesNotInclude=\(id)"
          )
          fallthrough  // reset anyway
        case .any:
          owner = .noneOf(ids: [id])
  ...
  mutating func handleElementAny(
    id: UInt?, filter: Filter,
    rule: FilterRule
  ) -> Filter {
    guard let id else {
      Logger.shared.error(
