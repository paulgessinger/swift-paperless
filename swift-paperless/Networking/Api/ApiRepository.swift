...
actor ApiRepository {
  ...
  init(connection: Connection) async {
    ...
    if let apiVersion, let backendVersion {
      Logger.api.notice(
        "Backend version info: API version: \(apiVersion), backend version: \(backendVersion.0).\(backendVersion.1).\(backendVersion.2)"
      )
  ...
  func request(url: URL) -> URLRequest {
    ...
    Logger.api.trace(
      "Creating API request for URL \(sanitizedUrl, privacy: .public), headers: \(request.allHTTPHeaderFields ?? [:])"
    )
    return request
  }
  ...
  private func fetchData(
    for request: URLRequest, code: Int = 200,
    ...
    if response.statusCode != code {
      let body = String(data: data, encoding: .utf8) ?? "[NO BODY]"
      Logger.api.error(
        "URLResponse to \(sanitizedUrl, privacy: .public) has status code \(response.statusCode) != \(code), body: \(body, privacy: .public)"
      )
  ...
  func fetchData<T: Decodable>(for request: URLRequest, as type: T.Type, code: Int = 200)
    async throws -> T
  {
    ...
    do {
      ...
      if Bundle.main.appConfiguration == .AppStore {
        ...
        Logger.api.error(
          "Unable to decode response to \(Self.sanitizeUrlForLog(url), privacy: .public) as \(T.self, privacy: .public) from body \(body, privacy: .public): \(error)"
        )
      }
      throw error
    }
...
extension ApiRepository: Repository {
  ...
  func download(documentID: UInt, progress: (@Sendable (Double) -> Void)? = nil) async throws
    -> URL?
  {
  ...
  func document(asn: UInt) async throws -> Document? {
    ...
    let endpoint = Endpoint.documents(
      page: 1, rules: [FilterRule(ruleType: .asn, value: .number(value: Int(asn)))])
