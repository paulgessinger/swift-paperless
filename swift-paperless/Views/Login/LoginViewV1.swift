      ...
      List {
        Section {
          NavigationLink {
            ...
            Label(
              String(localized: .login(.extraHeaders)), systemImage: "list.bullet.rectangle.fill")
          }
          NavigationLink {
...
struct LoginViewV1: LoginViewProtocol {
  ...
  private func login() async throws {
    ...
    do {
      ...
      Logger.shared.info(
        "Sending login request with headers: \(request.allHTTPHeaderFields ?? [:])")

      let session = URLSession(
        configuration: .default,
        delegate: PaperlessURLSessionDelegate(identity: viewModel.selectedIdentity),
        delegateQueue: nil)
      ...
      if statusCode != 200 {
        Logger.shared.error(
          "Token request response was not 200 but \(statusCode ?? -1, privacy: .public), \(String(decoding: data, as: UTF8.self))"
        )
        if statusCode == 400 {
  ...
  var body: some View {
    ...
    NavigationStack {
      Form {
        Section {
          HStack {
            ...
            case .valid:
              Label(
                String(localized: .login(.urlValid)),
                systemImage:
            ...
            case .error:
              Label(
                String(localized: .login(.urlError)),
                systemImage:
                  "xmark.circle.fill"
              )
              .labelStyle(.iconOnly)
          ...
          VStack(alignment: .leading) {
            ...
            if url.debouncedText.starts(with: "http://"),
              !LoginViewModel.isLocalAddress(url.debouncedText)
            {
              HStack(alignment: .top) {
                Image(systemName: "info.circle")
      ...
      .safeAreaInset(edge: .bottom) {
        ...
        .disabled(
          (!viewModel.loginStateValid || viewModel.username.isEmpty || viewModel.password.isEmpty)
            && (!identityBasedAuth || !viewModel.loginStateValid)
        )
        .padding()
