...
private struct StringArgView: View {
  ...
  var body: some View {
    VStack(alignment: .leading) {
      ...
      TextField(
        String(localized: .customFields(.queryArgumentLabel)),
        text: $stringValue)
    }
...
private struct FloatArgView: View {
  ...
  var body: some View {
    ...
    .task {
      if case .number(let val) = value {
        stringValue = String(val)
      } else if case .string(let val) = value, let doubleVal = Double(val) {
...
private struct IntegerArgView: View {
  ...
  var body: some View {
    ...
    .task {
      if case .integer(let val) = value {
        stringValue = String(val)
      } else if case .string(let val) = value, let intVal = Int(val) {
...
#Preview("Doc link") {
  @Previewable @State var expr = ExprContent(
    id: 9, op: .contains,
    arg: .array([
...
private struct SelectExactArgView: View {
  ...
  var body: some View {
    ...
    .task {
      if case .string(let val) = value,
        field.extraData.selectOptions.contains(where: { $0.id == val })
      {
        selected = val
      } else {
...
private struct SelectInArgView: View {
  ...
  var body: some View {
    ...
    .task {
      switch value {
      case .string(let val) where field.extraData.selectOptions.contains(where: { $0.id == val }):
        selected = [val]

      case .array(let arr):
        selected = arr.compactMap {
          if case .string(let id) = $0,
            field.extraData.selectOptions.contains(where: { $0.id == id })
          {
            return id
          }
          return nil
  ...
  private func unknownView() -> some View {
    ...
    return VStack {
      ...
      if Bundle.main.appConfiguration != .AppStore {
        ...
        Text(
          "This should be covered by the UI. **You're on TestFlight**: please send me feedback with the details:"
        )
...
private struct ExprView: View {
  ...
  private func updateField(_: UInt) {
    guard let field = store.customFields[expr.id] else {
      Logger.shared.error(
        "Expression set to field with id \(expr.id, privacy: .public) which was not found")
      return
    }
      ...
      if query != .any {
        ...
        Section {
          Button {
            ...
            Label(
              localized: .customFields(.clearCustomFieldFilterButtonLabel),
        ...
        Section {
          Button {
            ...
            Label(
              localized: .customFields(.addCustomFieldFilterButtonLabel),
...
struct CustomFieldFilterView: View {
  ...
  private func validate() async {
    ...
    guard query != emptyDefaultQuery else {
      Logger.shared.info(
        "Query is \(query.rawValue, privacy: .public), which is equivalent to .any, we'll apply that"
      )
      outputQuery = .any
      state = .valid
      return
    ...
    guard store.permissions.test(.view, for: .document) else {
      Logger.shared.info(
        "User does not have permission to view documents. Let them set filter without validation")
      outputQuery = query
      state = .valid
      return
    }
    ...
    do {
      ...
      Logger.shared.error(
        "Received error loading documents with filter, presenting error and not modifying actual filter state: \(error)"
      )
      state = .error
    }
  }
