...
extension Aspect where Content == Text {
  fileprivate init(_ label: String, systemImage: String) {
          ...
          HStack(alignment: .top, spacing: 25) {
            VStack(alignment: .leading) {
              ...
              if let id = document.correspondent, let name = store.correspondents[id]?.name {
                ...
                Aspect(
                  String(localized: .localizable(.correspondentNotAssignedPicker)),
                  systemImage: "person"
                )
              ...
              if let id = document.documentType, let name = store.documentTypes[id]?.name {
                ...
                Aspect(
                  String(localized: .localizable(.documentTypeNotAssignedPicker)),
                  systemImage: "doc"
                )
                .foregroundColor(Color.gray)
                .opacity(0.5)
            ...
            VStack(alignment: .leading) {
              ...
              if let id = document.storagePath, let name = store.storagePaths[id]?.name {
                ...
                Aspect(
                  String(localized: .localizable(.storagePathNotAssignedPicker)),
