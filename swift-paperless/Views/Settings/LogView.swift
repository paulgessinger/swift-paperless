...
struct LogView: View {
  ...
  private func loadLogs() async {
    ...
    let task = Task.detached {
      ...
      var entries =
        ...
        .map {
          Entry(date: $0.date, category: $0.category, message: $0.composedMessage, level: $0.level)
        }
  ...
  var body: some View {
    ScrollViewReader { proxy in
      List {
        if case .loaded(_, let file) = state, file == nil {
          Section {
        ...
        if case .loaded(let entries, _) = state, !entries.isEmpty {
          Section {
            Button {
        ...
        Section {
          ...
          case .loaded(let entries, _):
            ForEach(
              Array(zip(entries.indices, entries)),
              id: \.0
            ) { i, entry in
      ...
      .toolbar {
        ToolbarItemGroup(placement: .navigationBarTrailing) {
          if case .loaded(let entries, let file) = state {
            Button {
...
#Preview {
  NavigationStack {
    LogView(entries: [
      ...
      Entry(
        date: .now, category: "test",
        message: "this is yet another test log entry and it's even much more longer", level: .debug),
      Entry(date: .now, category: "TEST", message: "This is another test log entry", level: .error),
      Entry(date: .now, category: "TEST", message: "This is a test log entry", level: .info),
      Entry(date: .now, category: "TEST", message: "This is a test log entry", level: .info),
      Entry(date: .now, category: "TEST", message: "This is a test log entry", level: .info),
      ...
      Entry(date: .now, category: "TEST", message: "This is a test log entry", level: .info),
      Entry(
        date: .now, category: "test", message: "this is yet another test and as soon as it",
        level: .debug),
      Entry(date: .now, category: "TEST", message: "This is another test log entry", level: .error),
