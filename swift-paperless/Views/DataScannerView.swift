  ...
  private struct DataScannerViewInternal: UIViewControllerRepresentable {
    ...
    class Coordinator: NSObject, DataScannerViewControllerDelegate {
      ...
      func dataScanner(_: DataScannerViewController, didTapOn item: RecognizedItem) {
        guard case .barcode(let text) = item else {
      ...
      private func centerFromBounds(_ bounds: RecognizedItem.Bounds) -> CGPoint {
        CGPoint(
          x: (bounds.bottomLeft.x + bounds.bottomRight.x) / 2.0,
          y: (bounds.bottomLeft.y + bounds.bottomRight.y) / 2.0 + 50)
      }

      private func addHighlightView(
        id: UUID, text: String, center: CGPoint, dataScanner: DataScannerViewController
      ) {
      ...
      func dataScanner(
        _: DataScannerViewController, didRemove removedItems: [RecognizedItem],
        allItems _: [RecognizedItem]
      ) {
    ...
    static func dismantleUIViewController(
      _ uiViewController: DataScannerViewController, coordinator _: Coordinator
    ) {
      uiViewController.stopScanning()
    }
...
struct TypeAsnView: View {
  ...
  private var asn: UInt? {
    if (try? /\d+/.wholeMatch(in: text)) != nil {
      return UInt(text)
    }
    return nil
  }
  ...
  var body: some View {
    VStack {
      ...
      if case .valid(let document) = status {
