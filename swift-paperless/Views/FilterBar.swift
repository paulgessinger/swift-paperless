...
private struct FilterMenu<Content: View>: View {
  ...
  var body: some View {
    VStack {
      Menu {
        ...
        NavigationLink {
          ...
          Label(
            String(localized: .localizable(.savedViewsEditButtonLabel)), systemImage: "list.bullet")
        }
    ...
    .alert(
      Text(.localizable(.savedViewDeleteConfirmationTitle)), isPresented: $showDeletePrompt,
...
private struct Pill<Label: View>: View {
  ...
  var body: some View {
    ...
    .overlay(
      Capsule()
        .strokeBorder(
          active ? Color(.accent) : Color(.elementBorder),
          lineWidth: 0.66)
    )
      ...
      HStack {
        ...
        Menu {
          let isAdvancedSearch =
            !filterState.searchText.isEmpty && filterState.searchMode == .advanced
          let eligibleSortFields =
            (isAdvancedSearch || filterState.sortField == .score)
            ? SortField.allCases : SortField.allCases.filter { $0 != .score }
          ...
          Element(
            label: {
              Label(
                String(localized: .localizable(.sortMenuLabel)), systemImage: "arrow.up.arrow.down"
              )
              .labelStyle(.iconOnly)
