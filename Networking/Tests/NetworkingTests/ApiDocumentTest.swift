...
struct ApiDocumentTest {
  ...
  func testSetPermissionsKey() throws {
    ...
    let json = try JSONEncoder().encode(ApiDocumentUpdate(from: document))
    ...
    #expect(try absent("original_file_name"))
    #expect(try absent("archived_file_name"))
  }
  ...
  func testNilComponentsDecode(component: NilComponent) throws {
    ...
    let document = try decoder.decode(ApiDocument.self, from: withNil).domain
  ...
  func testNilComponentsEncode(component: NilComponent) throws {
    let data = try #require(testData("Data/Document/full.json"))
    var document = try decoder.decode(ApiDocument.self, from: data).domain

    #expect(document[keyPath: component.keyPath] != nil)
    document[keyPath: component.keyPath] = nil

    let json = try #require(
      String(data: JSONEncoder().encode(ApiDocumentUpdate(from: document)), encoding: .utf8))
  ...
  func testEncodeDocument() throws {
    ...
    let encoded = try JSONEncoder().encode(ApiDocumentUpdate(from: document))
  ...
  func testDecodeDocumentWithCustomFields() throws {
    ...
    #expect(document.owner == Owner.user(4))
