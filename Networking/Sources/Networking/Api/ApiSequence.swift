...
where Element: Decodable & Sendable {
  ...
  private func fixUrl(_ url: URL) -> URL {
    guard var components = URLComponents(url: url, resolvingAgainstBaseURL: false) else {
      Logger.networking.error(
        "Unable to decompose next-page URL for sequence URL fix, continuing with original URL")
      return url
    }
    ...
    guard let result = components.url else {
      Logger.networking.error(
        "Could not reassemble URL after sequence URL fix, continuing with original URL")
      return url
    }
  ...
  public func next() async throws -> Element? {
    ...
    guard !Task.isCancelled else {
      Logger.networking.notice(
        "API sequence (\(Element.self, privacy: .public) next task was cancelled.")
      return nil
    }
    ...
    guard let url = nextPage else {
      Logger.networking.notice(
        "\(Element.self, privacy: .public) API sequence has reached end (nextPage is nil)")
      hasMore = false
      return nil
    }
