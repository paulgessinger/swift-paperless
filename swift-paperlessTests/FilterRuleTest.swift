...
final class FilterRuleTest: XCTestCase {
  ...
  func testDecodingMultiple() throws {
    ...
    let result = try makeDecoder(tz: .init(abbreviation: "UTC")!).decode(
      [FilterRule].self, from: input)
  ...
  func testSearchModeConversion() {
    ...
    XCTAssertEqual(
      FilterState.SearchMode(ruleType: .titleContent), FilterState.SearchMode.titleContent)
  }
