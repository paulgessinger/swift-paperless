...
public struct Haptics: Sendable {
  ...
  public func notification(_ type: FeedbackType) {
      ...
      Task { @MainActor in
        HapticsInternal.shared.notificationGenerator.notificationOccurred(
          UINotificationFeedbackGenerator.FeedbackType(rawValue: raw)!)
      }
    #endif
  }
  ...
  public func impact(style: FeedbackStyle) {
    #if canImport(UIKit)
      let raw = style.rawValue
      Task { @MainActor in
        let style = UIImpactFeedbackGenerator.FeedbackStyle(rawValue: raw)!
