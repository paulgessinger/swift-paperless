...
private class LogRecordViewModel {
  public private(set) var state = LogRecordExportButton.LogState.none

  @ObservationIgnored
  public let change: ((LogRecordExportButton.LogState) -> Void)?

  public init(change: ((LogRecordExportButton.LogState) -> Void)? = nil) {
    self.change = change
  }
...
public struct LogRecordExportButton: View {
  ...
  public let content: ((LogState, @escaping () -> Void) -> AnyView)?

  public init(
    @ViewBuilder content: @escaping (LogState, @escaping () -> Void) -> some View,
    change: ((LogState) -> Void)? = nil
  ) {
  ...
  public init(change: ((LogState) -> Void)? = nil) {
    content = nil
    _viewModel = State(initialValue: LogRecordViewModel(change: change))
  }

  @ViewBuilder
  public static func loadingView() -> some View {
    Label(String(localized: .app(.logsExportLoading)), systemImage: "ellipsis")
