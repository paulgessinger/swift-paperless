...
extension Label where Icon == Image, Title == Text {
  ...
  public init(markdown: LocalizedStringResource, systemImage: String) {
    self.init(title: { Text(markdown) }, icon: { Image(systemName: systemImage) })
  }

  public init(localized: LocalizedStringResource, image: String) {
    self.init(String(localized: localized), image: image)
  }
}

public struct TightLabel: LabelStyle {
