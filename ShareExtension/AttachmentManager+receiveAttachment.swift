...
extension AttachmentManager {
  private static func loadItem(attachment: NSItemProvider, type: String) async throws -> URL {
    try await withCheckedThrowingContinuation { continuation in
      ...
      attachment.loadItem(
        forTypeIdentifier: type, options: nil,
        completionHandler: { data, error in
          ...
          case let data as NSData:
            Logger.shared.info(
              "Received data length \(data.count) bytes, writing to temporary file")
            ...
            do {
              ...
              Logger.shared.error(
                "Unable to store data to temporary file: \(error, privacy: .public)")
            }
