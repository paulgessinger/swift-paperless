...
struct UISettingsTest {
    ...
    func testApplyToCorrespondent() throws {
        let settingsPermissions = UISettingsPermissions(defaultOwner: 2, defaultViewUsers: [3], defaultViewGroups: [1], defaultEditUsers: [8], defaultEditGroups: [6])
    ...
    func testExplicitOwnerAndPermissionsOverrideSettingsDefaults() throws {
        ...
        let correspondent = TestCorrespondent(name: "Aaaaaa", owner: .user(1), permissions: Permissions(view: .init(users: [4], groups: [2]), change: .init(users: [9], groups: [7])))
    ...
    func testPartialPermissionOverrides() throws {
        ...
        let correspondent = TestCorrespondent(name: "Aaaaaa", owner: .unset, permissions: Permissions(view: .init(users: [4], groups: []), change: .init(users: [], groups: [7])))
        let applied = settingsPermissions.appliedAsDefaults(to: correspondent)
    ...
    func testExplicitNoneOwner() throws {
        ...
        let correspondent = TestCorrespondent(name: "Aaaaaa", owner: .none, permissions: Permissions(view: .init(users: [4], groups: [2]), change: .init(users: [9], groups: [7])))
        let applied = settingsPermissions.appliedAsDefaults(to: correspondent)
